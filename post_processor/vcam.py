...
class vcam:
    ...
    def update_M(self):
        ...
        Ry = np.array(
            [
                [math.cos(self.beta), 0, -math.sin(self.beta)],
                [0, 1, 0],
        ...
        Rz = np.array(
            [
                [math.cos(self.gamma), -math.sin(self.gamma), 0],
        ...
        self.K = np.array(
            [
                [-self.focus / self.sx, self.sh, self.ox],
                [0, self.focus / self.sy, self.oy],
                [0, 0, 1],
            ]
        )
        self.M1 = np.array(
            [[1, 0, 0, -self.Tx], [0, 1, 0, -self.Ty], [0, 0, 1, -self.Tz]]
        )
    ...
    def project(self, src):
        ...
        try:
            ...
            x_2 = (
                x_1 * K
                + 2 * self.KpCoeff[2] * x_1 * y_1
                + self.KpCoeff[3] * (r_2 + 2 * x_1**2)
            )
            y_2 = (
                y_1 * K
                + self.KpCoeff[2] * (r_2 + 2 * y_1**2)
                + 2 * self.KpCoeff[3] * x_1 * y_1
            )
