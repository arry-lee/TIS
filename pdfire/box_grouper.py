...
class TextBox(object):
    ...
    def __repr__(self):
        return '<TextBox %d %d %d %d %s %s>' %(self.left,self.top,self.right,self.bottom,self.text,self.direction)

    def __str__(self):
        return '%d;%d;%d;%d;@%s' %(self.left,self.top,self.right,self.bottom,self.text)
    ...
    def check_direction(self):
        # 检测文字旋转方向
        if self.text.isascii() and self.text.startswith('0') and self.direction=='t2b':
...
def hgroup(tblist):
    ...
    for k in d:
        ...
        while d[k]:
            ...
            self = stack[-1]
            if self.right == other.left and self.top == other.top and self.bottom == other.bottom:
                stack.append(other)
            else:
                if len(stack)==1:
                    # out.append(stack[0])
...
def vgroup(tblist):
    ...
    for k in d:
        ...
        while d[k]:
            ...
            self = stack[-1]
            if self.bottom == other.top and self.left == other.left and self.right == other.right:
                stack.append(other)
            else:
                if len(stack)==1:
                    # out.append(stack[0])
