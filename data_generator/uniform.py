...
from awesometable import AwesomeTable, _vstack, \
    from_dict, vstack, _hstack, hstack, wrap
...
class UniForm(object):
    ...
    def __init__(self, config):
        ...
        # self.table_rows = self.filters.get('max_num_of_rows',None)
        # Todo 防止出现单个汉字成一行导致的表格错位问题
        self.min_width_of_cell = self.filters.get('min_width_of_cell', 4)
        label_dir = config['base']['label_dir']
        print(label_dir)
        self.keys_dict, self.values_dict, self.texts, self.keys, self.values = key_value_generator(
            label_dir)
        ...
        if place_holder == 'key':
            ...
            if random.random() < self.empty_cell_ratio:  # 单元格空值的
                ...
                return random.choice(
                    wrap(self.texts, 10))  # Todo 将长文本按照10个字符宽度打包
            else:
    ...
    def rand_dict(self, n, cols, rows):
        if n > 1:
            d = {}
            for i in range(cols):
                d[self.rand_text(4, place_holder='key')] = self.rand_dict(n - 1,
                                                                          cols,
                                                                          rows)
            return d
        else:
            ...
            for i in range(cols):
                d[self.rand_text(4, place_holder='key')] = [
                    self.rand_text(4, place_holder='value') for _ in
                    range(rows)]
            return d

    def mxn(self, m, n, w, t=0):
        """表格的基本元生成"""
        rows = [[self.rand_text(4, place_holder='value') for _ in range(m)] for
                x in
                range(n)]
        tab = AwesomeTable()
        tab.table_width = w
    ...
    def single_key_multiple_values(self, **kwargs):
        ...
        for i in range(num):
            choices.append('[x]' + self.rand_text(random.randint(4, 8),
                                                  place_holder='value'))
    ...
    def multiple_rows_multiple_columns(self, **kwargs):
        ...
        table = [
            [self.rand_text(random.randint(2, 6), place_holder='value') for _ in
             range(cols)] for _ in
            range(rows)]
        ...
        if head_top:
            table[0] = [self.rand_text(random.randint(2, 6), place_holder='key')
                        for _
                        in range(cols)]
        if head_left:
            for i in range(rows):
