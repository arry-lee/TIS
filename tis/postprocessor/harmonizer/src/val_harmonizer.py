...
def load_iHarmony4_subset(dataset_dir, mode):
    ...
    with open(
        os.path.join(
            dataset_dir, "{0}_{1}.txt".format(dataset_dir.split("/")[-1], mode)
        ),
        "r",
    ) as f:
...
def calc_metrics(pred, gt, mask):
    ...
    fmse = (
        skimage.metrics.mean_squared_error(pred * mask, gt * mask)
        * total_pixels
        / fg_pixels
    )
    psnr = skimage.metrics.peak_signal_noise_ratio(
        pred, gt, data_range=pred.max() - pred.min()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pretrained", type=str, default="./pretrained/harmonizer.pth", help=""
    )
    parser.add_argument(
        "--datasets",
        type=str,
    ...
    print(
        "  - Metric Calculation Size: {0}".format(
            metric_size if args.metric_size > 0 else "original"
        )
    )
    ...
    for dkey, dvalue in datasets.items():
        ...
        for i, sample in enumerate(pbar):
            ...
            pbar.set_description(
                "MSE: {0:.4f}   fMSE: {1:.4f}   PSNR: {2:.4f}   SSIM: {3:.4f}".format(
                    metric["MSE"] / (i + 1),
                    metric["fMSE"] / (i + 1),
                    metric["PSNR"] / (i + 1),
                    metric["SSIM"] / (i + 1),
        ...
        print(
            "{0} - MSE: {1:.4f}   fMSE: {2:.4f}   PSNR: {3:.4f}   SSIM: {4:.4f}".format(
                dkey,
                metric["MSE"] / sample_num,
                metric["fMSE"] / sample_num,
                metric["PSNR"] / sample_num,
    ...
    print(
        "All - MSE: {0:.4f}   fMSE: {1:.4f}   PSNR: {2:.4f}   SSIM: {3:.4f}".format(
            mse, fmse, psnr, ssim
        )
    )
    print(
