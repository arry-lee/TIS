...
class Element(Rect):
    ...
    def show(self):
        ...
        self.render(drawer)
        im.crop(
            self.topleft + (self.right + self.line_width, self.bottom + self.line_width)
        ).show()
...
class Box(Element):
    ...
    def __str__(self):
        return super().__str__() + "{" + ",".join([str(i) for i in self.data]) + "}"
...
class Cell(Box):
    ...
    def __init__(self, initlist=None, align=None, padding_width=0, *args, **kwargs):
    ...
    def _do_align(self):
        if self.align == "mm":
            ...
            for text in self:
                text.xy = self.left + self._paddings[0], self.top + self._paddings[1]
                text.anchor = "lt"

        elif self.align == "rt":
            for text in self:
                text.xy = self.right - self._paddings[2], self.top + self._paddings[1]
                text.anchor = "rt"
