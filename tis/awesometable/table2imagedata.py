...
from awesometable.awesometable import (H_LINE_PATTERN, H_SYMBOLS,
                                       V_LINE_PATTERN,
                                       replace_chinese_to_dunder,
                                       str_block_width)
from awesometable.imagedata import Cell, ImageData, Table, draw_text, load_font, textbbox
...
def table2imagedata(
    ...
    for lno, line in enumerate(lines):
        ...
        if not cells:
            texts.append(draw_text((start, v), line, font_path, font_size, fill=fgcolor, anchor="lm"))
        ...
        for cno, cell in enumerate(cells):
            ...
            text = draw_text((start, v), cell, font_path, font_size, fill=fgcolor, anchor="lm")
            ...
            cell_boxes.add(cbox)
            table_record = (min(table_record[0], cbox[0]), min(table_record[1], cbox[1]), max(table_record[2], cbox[2]),
    ...
    for t in table_cell_dict:
        ...
        for box in table_cell_dict[t]:
            cell = Cell(box[0], box[1], box[2] - box[0], box[3] - box[1], outline=bdcolor, line_width=line_width)
